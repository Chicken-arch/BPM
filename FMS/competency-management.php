    ...
    <?php if ($activeTab == 'gap-analysis'): ?>
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <!-- Button for requesting employee records from HR4 -->
                <button onclick="requestHR4Records()" class="flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-all shadow-md font-bold text-sm transform hover:scale-105 active:scale-95">
                    <i class="fas fa-file-import"></i>
                    Request Records from HR4
                </button>
            </div>
            ...
            function requestHR4Records() {
                // Open the request modal instead of direct sync
                const modal = document.getElementById('hr4RequestModal');
                modal.classList.remove('hidden');
                document.body.style.overflow = 'hidden';
            }

            function closeHR4RequestModal() {
                const modal = document.getElementById('hr4RequestModal');
                modal.classList.add('hidden');
                document.body.style.overflow = 'auto';
                document.getElementById('hr4RequestForm').reset();
            }

            function submitHR4Request() {
                const form = document.getElementById('hr4RequestForm');
                ...
                formData.append('action', 'manual_hr4_request');

                Swal.fire({
                    title: 'Sending Request to HR4...',
                ...
                .catch(err => {
                    Swal.fire('Error', 'Failed to connect to HR4 server.', 'error');
                });
            }
            </script>
            ...
            <!-- Manual HR4 Request Modal -->
            <div id="hr4RequestModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
                <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                    <div class="p-6 border-b border-gray-200 flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-xl font-bold text-gray-900">Request Record from HR4</h3>
                        </div>
                        <button onclick="closeHR4RequestModal()" class="text-gray-400 hover:text-gray-600">
                            <i class="fas fa-times text-xl"></i>
                        </button>
                    </div>
                    <form id="hr4RequestForm" class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Employee ID *</label>
                                <input type="text" name="employee_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500" placeholder="e.g. 0001">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Full Name *</label>
                                <input type="text" name="employee_name" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500" placeholder="e.g. John Smith">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Department *</label>
                                <input type="text" name="department" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500" placeholder="e.g. Operations">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Position *</label>
                                <input type="text" name="position" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500" placeholder="e.g. Logistics Coordinator">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Status *</label>
                                <select name="talent_type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500">
                            ...
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Gap % *</label>
                                <div class="relative">
                                    <input type="number" name="gap_percentage" min="0" max="100" value="0" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 pr-10">
                    ...
                    <div class="p-6 border-t border-gray-200 flex justify-end gap-3">
                        <button onclick="closeHR4RequestModal()" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold">Cancel</button>
                        <button onclick="submitHR4Request()" class="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-bold shadow-lg flex items-center gap-2">
                            <i class="fas fa-paper-plane"></i>
                            Submit Request to HR4
                        </button>
                    </div>
                </div>
            </div>
