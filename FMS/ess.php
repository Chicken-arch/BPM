    ...
    <?php if ($activeTab == 'payroll'): ?>
        <div class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <?php 
                $totalPaid = 0;
                foreach($myPayslips as $p) if($p['status'] === 'Paid') $totalPaid += $p['net_pay'];
                ?>
                <div class="bg-indigo-600 rounded-2xl p-6 text-white shadow-lg shadow-indigo-100">
                    <p class="text-indigo-100 text-xs font-bold uppercase tracking-wider mb-1">Total Net Received</p>
                    <p class="text-3xl font-black"><?php echo formatCurrency($totalPaid); ?></p>
                </div>
                <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                    <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Latest Payslip Date</p>
                    <p class="text-2xl font-bold text-gray-800"><?php echo !empty($myPayslips) ? htmlspecialchars($myPayslips[0]['month']) : 'N/A'; ?></p>
                </div>
                <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                    <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-1">Total Records</p>
                    <p class="text-2xl font-bold text-gray-800"><?php echo count($myPayslips); ?></p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow border border-gray-100 p-6">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                        <div class="bg-indigo-600 h-full rounded-full leave-bar transition-all duration-1000 ease-out" style="width: 0%" data-target="<?php echo $balance['annual_total'] > 0 ? ($balance['annual_remaining'] / $balance['annual_total']) * 100 : 0; ?>"></div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow border border-gray-100 p-6">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                        <div class="bg-emerald-600 h-full rounded-full leave-bar transition-all duration-1000 ease-out" style="width: 0%" data-target="<?php echo $balance['sick_total'] > 0 ? ($balance['sick_remaining'] / $balance['sick_total']) * 100 : 0; ?>"></div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow border border-gray-100 p-6">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden">
                        <div class="bg-amber-600 h-full rounded-full leave-bar transition-all duration-1000 ease-out" style="width: 0%" data-target="<?php echo $balance['personal_total'] > 0 ? ($balance['personal_remaining'] / $balance['personal_total']) * 100 : 0; ?>"></div>
                    </div>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                        ...
                        <button onclick="showRequestLeaveModal()" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-bold transition-all shadow-md shadow-indigo-100 flex items-center gap-2 text-sm">
                                    ...
                                    <tr class="hover:bg-gray-50/80 transition-all group">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="text-sm font-bold text-gray-900"><?php echo htmlspecialchars($personalInfo['full_name']); ?></div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                <div class="text-xs font-bold text-gray-900"><?php echo date('M d, Y', strtotime($date)); ?></div>
                                                <div class="text-[10px] text-gray-400 font-medium">(<?php echo date('l', strtotime($date)); ?>)</div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <?php if ($data['leave']): ?>
                                            ...
                                            <?php elseif ($data['schedule']): ?>
                                                <span class="text-xs font-medium text-gray-700">Regular Duty (<?php echo htmlspecialchars($data['schedule']['shift_name']); ?>)</span>
                                            <?php else: ?>
                                                <span class="text-xs text-gray-400 italic">No Activity</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            ...
                                            $badgeClass = '';
                                            switch($data['status']) {
                                                case 'Present': $badgeClass = 'bg-emerald-50 text-emerald-700 border-emerald-100'; break;
                                                case 'Absent': $badgeClass = 'bg-red-50 text-red-700 border-red-100'; break;
                                                case 'Late': 
                                                case 'Undertime':
                                                case 'Late & Undertime':
                                                    $badgeClass = 'bg-amber-50 text-amber-700 border-amber-100'; break;
                                                case 'Leave': $badgeClass = 'bg-blue-50 text-blue-700 border-blue-100'; break;
                                                default: $badgeClass = 'bg-gray-50 text-gray-400 border-gray-100';
                                            }
                                            ?>
                                            <span class="px-3 py-1.5 rounded-full text-[9px] font-black uppercase border <?php echo $badgeClass; ?> shadow-sm inline-flex items-center gap-1.5">
                                                <span class="w-1.5 h-1.5 rounded-full current-color"></span>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-xl shadow border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-8 text-center bg-gradient-to-br from-white to-gray-50/30">
                        <div class="text-4xl font-black text-gray-900 mb-1 tabular-nums" id="live_clock">00:00:00</div>
                        <p class="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-8"><?php echo date('l, M d Y'); ?></p>
                        
                        <?php if (!$isAdmin): ?>
                            <div class="flex flex-col gap-3">
                                <?php if ($canClockIn): ?>
                                    <form method="POST" action="ess.php?tab=leave" class="w-full">
                                        <input type="hidden" name="action" value="attendance_clock_in">
                                        <button type="submit" class="w-full py-4 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-all font-black uppercase tracking-widest shadow-lg shadow-emerald-100 flex items-center justify-center gap-3 active:scale-95">
                                ...
                                <?php if ($canClockOut): ?>
                                    <form method="POST" action="ess.php?tab=leave" class="w-full">
                                        ...
                                        <button type="submit" class="w-full py-4 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-all font-black uppercase tracking-widest shadow-lg shadow-red-100 flex items-center justify-center gap-3 active:scale-95">
                        ...
                        <?php else: ?>
                            <div class="p-4 bg-white/60 border border-gray-100 rounded-xl text-[10px] text-gray-400 italic">Admin accounts use management portals for time tracking.</div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <script>
                    ...
                    document.addEventListener('DOMContentLoaded', () => {
                        setTimeout(() => {
                            document.querySelectorAll('.leave-bar').forEach(bar => {
